...
function RegisterDetailsContent() {
  ...
  const {
    ...
    formState: { errors: detailsErrors, isSubmitting: isRegistering },
  } = useForm<RegisterDetailsInput>({
    resolver: zodResolver(registerDetailsSchema),
  });

  const onRegisterDetails = async (data: RegisterDetailsInput) => {
    if (!email) {
  ...
  return (
    <div className="space-y-8 bg-white dark:bg-gray-900 p-8 rounded-xl border border-gray-200 dark:border-gray-700 hover:border-indigo-400 dark:hover:border-indigo-500 hover:shadow-md transition-all">
      ...
      <form className="mt-8 space-y-6" onSubmit={handleDetailsSubmit(onRegisterDetails)}>
        <div className="space-y-4">
          <div>
            ...
            {detailsErrors.fullName && (
              <p className="mt-1 text-sm text-red-600">{detailsErrors.fullName.message}</p>
            )}
          </div>
          ...
          <div>
            ...
            {detailsErrors.password && (
              <p className="mt-1 text-sm text-red-600">{detailsErrors.password.message}</p>
            )}
          </div>
          ...
          <div>
            ...
            {detailsErrors.confirmPassword && (
              <p className="mt-1 text-sm text-red-600">{detailsErrors.confirmPassword.message}</p>
            )}
          </div>
        </div>
